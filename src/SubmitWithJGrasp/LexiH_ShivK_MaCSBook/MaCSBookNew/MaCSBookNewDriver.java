		...
		while (true) {
			...
			try {
				...
				if (choice == 1) {
					...
					/**
					 * Gets the new student's final mark. Fully error trapped to ensure only actual
					 * numbers are entered, which are within 0 and 100 inclusive (Percent)
					 */
					while (true) {
						try {
							System.out.print("\nEnter the new student's average mark: ");
					...
					/**
					 * Outputs the student's current average and the class's average alongside the
					 * user's data entries.
					 */
					try {
				 ...
				 */
					...
					while (true) {
						...
						try {
							...
							if (choice > 1) {
								String oldMark = book.getStudentInfo(index).strip().split(" ")[2 + 3 * choice];
								System.out.println("old mark: " + oldMark + "%.");
								System.out
										.println("\nThe student's current average is "
												+ String.format("%.1f",
														(Double.parseDouble(value) + book.getStudentAverage(index)
																- Double.parseDouble(
																		oldMark.substring(0, oldMark.length() - 1))))
												+ "%.");
								System.out
										.println("The current class average is "
												+ String.format("%.1f",
														(book.getClassAverage(true) * book.students
																+ Double.parseDouble(value)) / (book.students + 1.0))
												+ "%.");
							}
							/**
							 * Updating student data from file
